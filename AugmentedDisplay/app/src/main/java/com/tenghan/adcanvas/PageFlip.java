...
public class PageFlip {
     ...
     */
    public PageState getFlipState() {
        return mFlipState;
    }


    public void onSurfaceCreated() throws PageException {
    ...
    public void onFingerDown(float touchX, float touchY) {
        ...
        if (isContained) {
            ...
            mFlipState = PageState.BEGIN_FLIP;
        }
    }
    ...
    public boolean onFingerMove(float touchX, float touchY) {
        ...
        // begin to move
        if (mFlipState == PageState.BEGIN_FLIP &&
            ...
            if (mPages[SECOND_PAGE] == null &&
                ...
                mFlipState = PageState.BACKWARD_FLIP;
            }
            else if (mListener != null &&
                    mListener.canFlipForward() &&
                    (dx < 0 && originP.x > 0 || dx > 0 && originP.x < 0)) {
                mFlipState = PageState.FORWARD_FLIP;
            }
        }

        // in moving, compute the TouchXY
        if (mFlipState == PageState.FORWARD_FLIP ||
                mFlipState == PageState.BACKWARD_FLIP ||
                mFlipState == PageState.RESTORE_FLIP) {
            ...
            // finger point
            if (PageState.FORWARD_FLIP == mFlipState) {
                dx *= 1.2f;
            }
            else {
                dx *= 1.1f;
    ...
    public boolean onFingerUp(float touchX, float touchY, int duration) {
        ...
        // backward flipping
        else if (mFlipState == PageState.BACKWARD_FLIP) {
        ...
        // ready to flip
        else if (mFlipState == PageState.BEGIN_FLIP) {
            mIsVertical = false;
            mFlipState = PageState.END_FLIP;
    ...
    public boolean canAnimate(float touchX, float touchY) {
        return (mFlipState == PageState.FORWARD_FLIP &&
    ...
    public boolean animating() {
        ...
        if (isAnimating) {
            ...
            // continue to flip
            if (mFlipState == PageState.BACKWARD_FLIP ||
                    mFlipState == PageState.RESTORE_FLIP) {
            ...
            else if (mFlipState == PageState.FORWARD_FLIP) {
    ...
    public void abortAnimating() {
        ...
        if (mFlipState == PageState.FORWARD_FLIP) {
            mFlipState = PageState.END_WITH_FORWARD;
        }
        else if (mFlipState == PageState.BACKWARD_FLIP) {
            mFlipState = PageState.END_WITH_BACKWARD;
        }
        else if (mFlipState == PageState.RESTORE_FLIP) {
            mFlipState = PageState.END_WITH_RESTORE;
        }
    }
    ...
    public boolean isStartedFlip() {
        return mFlipState == PageState.BACKWARD_FLIP ||
                mFlipState == PageState.FORWARD_FLIP ||
                mFlipState == PageState.RESTORE_FLIP;
    }
    ...
    public boolean isEndedFlip() {
        return mFlipState == PageState.END_FLIP ||
                mFlipState == PageState.END_WITH_RESTORE ||
                mFlipState == PageState.END_WITH_BACKWARD ||
                mFlipState == PageState.END_WITH_FORWARD;
    }
    ...
    private void createGradientShadowTexture() {
        ...
        Bitmap shadow = PageUtils.createGradientBitmap();
